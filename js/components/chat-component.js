export const ChatComponent = {
            ...
            <aside id="sidebar-panel" class="w-full md:w-[360px] h-full flex flex-col border-r border-zinc-900 bg-black z-20 transition-all duration-300">
                <div class="p-5 flex items-center justify-between border-b border-zinc-900">
                    <div class="flex items-center space-x-3">
                        <div class="relative">
                            <img id="current-user-avatar" src="assets/images/default-avatar.svg" class="w-9 h-9 rounded-full object-cover ring-1 ring-zinc-800">
                    ...
                    <header class="p-4 bg-black/80 backdrop-blur-md flex items-center justify-between border-b border-zinc-900 z-10">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <span id="active-chat-status" class="text-[10px] text-zinc-500 font-medium tracking-wide">Active now</span>
                            </div>
                        </div>
                    </header>
    ...
    renderChatItem(chat, currentUserId) {
        ...
        // WhatsApp jaisa big pill nahi—Insta style premium blue dot
        ...
        // Unread hone par raw bold text look
        ...
        <div data-chat-id="${chat.id}" class="chat-inbox-row p-3 flex items-center justify-between rounded-xl hover:bg-zinc-900/40 cursor-pointer transition-all group">
            <div class="flex items-center space-x-3 flex-1 min-w-0">
                <img src="${chat.targetUser.photoURL || 'assets/images/default-avatar.svg'}" class="w-12 h-12 rounded-full object-cover border border-zinc-900">
                <div class="flex-1 min-w-0">
                    <div class="flex items-center justify-between">
                        <p class="text-xs ${nameStyleClass} tracking-tight truncate">${chat.targetUser.displayName}</p>
                    ...
                    <p class="truncate mt-0.5 ${textStyleClass}">${chat.lastMessage || 'Sent a message...'}</p>
                </div>
            </div>
            ${dotHTML}
        </div>`;
    ...
    renderMessageItem(msg, currentUserId) {
        ...
        // Super micro clean icons for premium aesthetic
        if (isMe) {
            if (msg.seen === true) {
                // 🔵 Micro Blue Seen Ticks
                ticksHTML = `
        ...
        // Instagram uses organic capsule-like rounding for blocks (rounded-2xl)
        ...
        <div class="flex w-full ${isMe ? 'justify-end' : 'justify-start'}">
            <div class="max-w-[70%] px-4 py-2.5 shadow-sm font-normal tracking-wide text-[13px] leading-snug break-words ${bubbleStyle}">
                <p>${msg.text}</p>
